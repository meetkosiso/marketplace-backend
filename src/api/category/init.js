...
export async function initCategory(req, res) {
  ...
  cat1.save().then(result => console.log(result));
  const cat2 = new Category(category2);
  cat2.save().then(result => console.log(result));
...
export function getAnyCategory() {
  Category.findOne().sort({ created_at: -1 }).exec((_err, result) => result);
}
