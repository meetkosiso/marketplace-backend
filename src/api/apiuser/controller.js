...
import Apiuser, { ObjectId } from "./model";
...
export function findVerify(req, res) {
  ...
  return Apiuser.count({ attribute: value })
    .then((result) => {
      console.log(`\r\n Result count: ${result}`);
...
export function findOne(req, res) {
  ...
  return Apiuser.findById(recordId)
    .then((result) => {
...
export async function update(req, res) {
  ...
  if (data.email) newObject.email = data.email.toLowerCase();
  ...
  return Apiuser.findByIdAndUpdate(userId, newObject, { new: true })
    .then((result) => {
...
export async function modify(req, res) {
  ...
  const record = await Apiuser.findById(recordId).exec();
  ...
  // Find record and update it with id
  return Apiuser.findByIdAndUpdate(recordId, newObject, { new: true })
    .then((result) => {
...
export async function destroy(req, res) {
  ...
  return Apiuser.findByIdAndRemove(recordId)
    .then((record) => {
...
export function countAdmin(condition) {
  return new Promise((resolve, reject) => {
    Apiuser.countDocuments(condition)
      .exec((err, result) => {
        if (err) throw err;
