...
import { findCurrencyByCode } from "../../api/currency/controller";
...
export function getProperty(givenObject, givenProperty, ...args) {
  ...
  let value;
  switch (args.length) {
    case 0: value = givenObject[givenProperty]; break;
    case 1: value = givenObject[givenProperty][args[0]]; break;
    case 2: value = givenObject[givenProperty][args[0]][args[1]]; break;
...
export function propertyExist(
  ...
  if (dirDirChild === null) {
    ...
    checker = hasProp(mainObject, dirChild)
      && hasProp(mainObject[dirChild], dirDirChild)
      && hasProp(mainObject[dirChild][dirDirChild], dirDirDirChild)
      && hasProp(mainObject[dirChild][dirDirChild][dirDirDirChild], dirDirDirDirChild);
...
export async function getCurrentUnitPrice(product, quantity, unitPrice) {
  ...
  switch (product.price.valuation) {
    ...
    case "LIFO":
      ...
      break;
    case "AVCO":
      currentStockAvailable = (parseInt(product.available, 10) + parseInt(quantity, 10));
      totalPrice = (
        (parseInt(product.available, 10) * parseInt(product.price.unitPrice, 10))
...
export function findEmail(User, email) {
  return new Promise((resolve, reject) => {
    User.findOne({ email, action: "allow" })
      .exec((err, result) => {
        if (err) reject(err);
